    ...
    <section>
      <div className='space-y-2'>
        <div className='flex items-center justify-between mr-16'>
          <h1 className='text-2xl text-bold'>Products_49</h1>
          <Button asChild variant='default'>
            <Link href='/store_49/admin_49/products_49/create'>
              Create Products
            </Link>
          </Button>
        </div>
          ...
          {items.map((item) => {
            ...
            return (
              <TableRow key={productId}>
                <TableCell>
                  <Link href={`/store_49/product_49/${productId}`}>{name}</Link>
                </TableCell>
                <TableCell>{company}</TableCell>
                <TableCell>{formatCurrency(price)}</TableCell>
                <TableCell className='flex items-center gap-x-2'>
                  <Link
                    href={`/store_49/admin_49/products_49/${productId}/edit`}
                  >
